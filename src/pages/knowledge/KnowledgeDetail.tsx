...
const knowledge: Knowledge = 
  {
    ...
    appreciations: [
      {user_id: 1, user_name: "tarp", img: logo, content: "まじでありがとう！POSSE最後までいるって決断できました！！"},
      {user_id: 2, user_name: "kashiken", img: "https://images.unsplash.com/photo-1551963831-b3b1ca40c98e", content: "まじでありがとう！POSSE最後までいるって決断できました！！"},
      {user_id: 3, user_name: "tsuyoshi", img: logo, content: "まじでありがとう！POSSE最後までいるって決断できました！！"},
    ],
    transactions: [
      {id: 1, book_date:"2022/02/11", points: 1000, booker: "kashiken", hold_status: "開催済み"},
      {id: 2, book_date:"2022/02/11", points: 1000, booker: "kashiken", hold_status: "開催済み"},
      {id: 3, book_date:"2022/02/11", points: 1000, booker: "kashiken", hold_status: "開催済み"},
      {id: 4, book_date:"2022/02/11", points: 1000, booker: "kashiken", hold_status: "開催済み"},
      {id: 5, book_date:"2022/02/11", points: 100, booker: "kashiken", hold_status: "開催済み"},
      {id: 1, book_date:"2022/02/11", points: 1000, booker: "kashiken", hold_status: "開催済み"},
      {id: 2, book_date:"2022/02/11", points: 500, booker: "kashiken", hold_status: "開催済み"},
      {id: 3, book_date:"2022/02/11", points: 1000, booker: "kashiken", hold_status: "開催済み"},
      {id: 4, book_date:"2022/02/11", points: 2000, booker: "kashiken", hold_status: "開催済み"},
      {id: 5, book_date:"2022/02/11", points: 3000, booker: "kashiken", hold_status: "開催済み"},
    ]
  };
    ...
    <div className={styles.root}>
      ...
      <Box sx={{display: "flex"}}>
        <Box sx={{display: "flex", width: "50%"}}>
          ...
          <Box className={styles.big_picture}>
            ...
            <img src={knowledge.img[index]} height="100%" width="100%" style={{objectFit:'contain', backgroundColor: "#F5F5F5"}} alt={`Image ${index}`}/>
        ...
        <Box sx={{ml: "70px", width: "40%"}} >
          <h1 style={{ margin: 0, fontSize: "28px", color: "#666666" }}>{knowledge.title}</h1>
          <Typography sx={{ my: 2, fontSize: "20px" }}>{knowledge.price}pt</Typography>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2}}>
            ...
            <Box onClick={() => {commentsClickScroll(); transactionsClickScroll();}} sx={{ display: 'flex', flexDirection: "column", alignItems: 'center', gap: 1}}>
              <ChatBubbleOutlineIcon sx={{width: 28, height: 28, "@media screen and (max-width:498px)": {width: 25, height: 25} }} />
              <Typography sx={{ fontSize: 10, "@media screen and (max-width:498px)": {fontSize: 12}, color: '#666666' }} >
                コメント
              </Typography>
            </Box>
            <Box onClick={() => {commentsClickScroll(); transactionsClickScroll();}} sx={{ display: 'flex', flexDirection: "column", alignItems: 'center', gap: 1}}>
              <CelebrationIcon sx={{width: 30, height: 30, "@media screen and (max-width:498px)": {width: 25, height: 25} }} />
              <Typography sx={{ fontSize: 10, "@media screen and (max-width:498px)": {fontSize: 12}, color: '#666666' }} >
                感謝の言葉
              </Typography>
            </Box>
      ...
      <Box id="comments" sx={{pt: 7}}>
        <SimpleTabs labels={['コメント', '感謝の言葉', '取引履歴']}>
          <div id="comments">
            <Typography sx={{ mt: 3, mb: 1, fontSize: "20px", fontWeight: "bold", color: "#666666" }}>コメント({knowledge.comments?.length ?? 0})</Typography>
            <Box sx={{ display: 'flex', "@media screen and (max-width:498px)": {flexDirection: "column"}}} >
              <Box>
                {(knowledge.comments ?? []).map((comment: { user_id: number; user_name: string; img: string; content: string; })  => (
                <Box sx={{ display: 'flex'}}>
              ...
              <Stack component="form" noValidate 
                ...
                />
                <Button onClick={() => showSnackbar('コメントを送信しました！', 'success')} variant="outlined" type="submit" style={{ width: "50%", fontWeight: "bold" , padding: "10px 0" ,marginTop: "20px"}} >
                  コメントを送信する
                </Button>
              </Stack>
            </Box>
          ...
          <div id="appreciations">
            <Typography sx={{ mt: 3, mb: 1, fontSize: "20px", fontWeight: "bold", color: "#666666" }}>感謝の言葉({knowledge.appreciations?.length ?? 0})</Typography>
            <Box>
              {(knowledge.appreciations ?? []).map((appreciation: { user_id: number; user_name: string; img: string; content: string; })  => (
                <Box sx={{ display: 'flex'}}>
                  <Link to={'../user/' + appreciation.user_id} style={{ textDecoration: 'none' }}>
                    <Avatar src={appreciation.img}  sx={{ mr: 1, width: 40, height: 40 ,"@media screen and (max-width:498px)": {width: 25, height: 25} }} alt="PeerPerk" />
                  </Link>
                  <Link to={'../user/' + appreciation.user_id} style={{ textDecoration: 'none' }}>
                    <Box sx={{ display: 'flex', flexDirection: "column"}}>
                      <Typography sx={{ fontWeight: "bold", fontSize: 16, "@media screen and (max-width:498px)": {fontSize: 14}, color: '#666666' }} >
          ...
          <div id="transactions">
            <Typography sx={{ mt: 3, mb: 2, fontSize: "20px", fontWeight: "bold", color: "#666666" }}>取引履歴({knowledge.transactions?.length ?? 0})</Typography>
